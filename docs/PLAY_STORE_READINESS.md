...
### P1. 개인정보처리방침 공개 URL — **해결됨 (2026-09-21)**

GitHub Pages로 정적 페이지를 게시했습니다: **https://seonggi.github.io/baby_check/privacy-policy.html**
(`privacy-policy.html`을 저장소 루트에 추가 → `master` 브랜치 push →
`gh api repos/SeongGi/baby_check/pages`로 Pages 활성화, source: `master` `/`).
로그인 없이 열리는 것을 `curl -sI` HTTP 200으로 확인. `src/screens/Profile.tsx`의
"개인정보처리방침 전문 보기 🔗" 버튼이 이 URL을 엽니다.
(처음엔 claude.ai Artifact로 게시했는데 기본적으로 비공개라 로그인을
요구해서, 공개 접근이 확실한 GitHub Pages로 교체했습니다.)

**남은 일**: 이 URL을 Play Console 스토어 등록정보의 "개인정보처리방침" 필드에
입력해야 하지만, P2와 같은 이유로 그 입력 화면도 지금은 못 찾았습니다.
아마 같은 "프로덕션 신청" 흐름에서 함께 물어볼 가능성이 높습니다. URL만
미리 확정해 두고 대기.

### P2. Play Console "Data safety(데이터 보안)" 설문 — **지금 단계에선 접근 불가, 차단 아님으로 재분류**

2026-09-21: 사용자와 함께 Play Console 전체 메뉴(대시보드/통계/게시 개요/
Google Play로 보호됨/테스트 및 출시/모니터링 및 개선/사용자 늘리기/수익
창출)를 샅샅이 확인했지만 "앱 콘텐츠"나 "데이터 보안"으로 가는 별도 메뉴가
**존재하지 않았습니다.**

Data safety는 데이터를 전혀 수집하지 않는 앱도 반드시 제출해야 하는 필수
항목이라 "필요 없어서 생략됨"은 아닙니다. 가장 유력한 설명: 대시보드의
"프로덕션 액세스 신청" 카드에 "신청 시 비공개 테스트에 관한 몇 가지 질문에
답해야 합니다"라는 안내와 "질문 미리보기" 링크가 있었는데, **이 계정 구조에서는
Data safety를 포함한 콘텐츠 신고가 별도 메뉴가 아니라 "프로덕션 신청" 버튼을
누를 때 나오는 질문 흐름에 포함되어 있는 것으로 보입니다.** 지금은 테스터
요건(P5) 미충족으로 그 버튼 자체가 비활성 상태라 질문이 안 보이는 것.

**결론**: 지금 단계에서 더 찾을 필요 없음. 이 앱이 8월에 실제 배포된 이력이
있어(그때는 이런 절차를 거쳤을 것), 이미 완료됐거나 프로덕션 신청 시점에
자연스럽게 나타날 것으로 예상. 아기 이름/태명, 생년월일, 출생 체중,
수유·배변·목욕·체중 기록을 수집하고, 기기 로컬 우선 저장 후 가족 연결 시
Firestore에 동기화하며, 제3자 공유 없음·HTTPS 암호화 전송이라는 사실만
정리해 두면, 질문이 나타났을 때 바로 답할 수 있음.
...
2. ~~Android 개발자 인증~~ — **해결됨 (6절 참고, 아기기록은 이미 등록돼 있어
   추가 조치 불필요)**
3. **가장 시급, 사용자 행동 필요**: 비공개 테스트 opt-in 링크(P5의 Android/웹
   링크)를 테스터 21명(최소 12명)에게 보내 "테스트 참여"를 누르게 한다.
   **14일 카운트는 이 시점부터 시작되므로 가장 먼저 처리할수록 유리합니다.**
4. 공개 개인정보처리방침 URL을 Play Console 스토어 등록정보에 입력 (P1,
   URL은 이미 있음: https://seonggi.github.io/baby_check/privacy-policy.html)
5. Play Console Data safety 설문 작성 (P2, "프로덕션 신청" 흐름에서 함께
   나타날 가능성 높음)
